...
- 工程修复（v0.5.16-v0.5.17）：v0.5.16 修复平台对话/presence 时区、ShareGate 受管 LLM 预算、兴趣同批累加、夜间复盘后置动作原子性与 WebUI `share_note` 非法输入；v0.5.17 统一 `message_in` / `reply_out` / `conversation_end` 事件时间戳到 persona 本地注入时钟并稳定时区测试。
