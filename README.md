...
- 每实例独立生活（现状 v0.2.7）：`life_personas` 白名单，每个 persona 拥有独立的档案、兴趣、情绪 scope 与分享记录；v1.1 起同人格多实例共享同一 SQLite 档案（见 `docs/design.md` 多实例并发）。
...
- 生态联动（现状 v0.2.7）：可选接入 ESM 读取精力/情绪、施加信号；缺失时静默降级；v1.1 计划内新契约不再默认降级。
