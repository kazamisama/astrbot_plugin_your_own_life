...
| L2 | v2 | 结构性能力（硬依赖） | 统一记忆库、实体关系图、关注对象、故地重游、记忆温度、回顾/胶囊/评估、LLM 自主更改+回滚、多实例租约（全部已实现） |
...
- 依赖：ESM `consume_energy`（v0.11.0 已落地，见 `docs/requirements.md`）；ESM 缺失时按验收要求退化为只读 gate + 本地估算并显式标注。
- 模块：`life/esm_adapter.py`、`life/browser.py`、`life/db.py`（`daily_usage.energy_used`）。
- 配置：`energy_budget`（每日上限，0 = 无上限）、`energy_gate` 沿用。
...
- 依赖：L1-08/09 视图（L1-10 时间轴随后单独落地）。
...
- 依赖：ShareGate（L0）。
- 模块：`life/share.py`。
- 配置：`share_silence_rate`（默认 0.15，0-1；手动分享 `force=True` 绕过）。
- 验收：沉默不写分享日志（已满足，仅写 `share_silent` 状态快照，note 标记 dropped）；日记可体现“今天不想说话”（已满足，快照进入夜间复盘素材）。
...
- 依赖：统一记忆库 + 关注对象（L2-10）。
- 模块：`life/browser.py`（`run_revisit`，夜间复盘调用）、`life/db.py`（`revisit_of` 列、`list_revisit_candidates` / `list_notes_by_url_hash` / `mark_note_revisit` / `list_revisit_chains`，旧库唯一约束迁移）、`life/prompts.py`（`build_revisit_prompt`）、`life/webui.py` + `pages/life/index.html`（`GET /revisit_chains` 与故地重游 tab）。
- 配置：`revisit_interval_days`（默认 30）。
- 验收：到期自动触发 revisit（已满足，夜间复盘按阈值候选并逐个生成）；新短记带 `revisit` 标记并引用原链接（已满足，`revisit_of` 指向原短记，`source=revisit/*`，事件幂等键 `revisit/{original_id}`）；原短记与后续状态可串联查看（已满足，WebUI 故地重游 tab 展示原短记 + 后续短记链）。
- 命名区分：与 L1-02 旧事新感（日记回看）不同，本项是对链接/项目的定期重访。

### L2 小结

- L2-01 至 L2-11 全部已实现（v0.5.0-v0.5.10）；L2 结构性能力（统一记忆库 / 实体图 / 记忆温度 / 回顾 / 时间胶囊 / 分享沉默率 / 季度评估 / 自主更改回滚 / 多实例租约 / 关注对象 / 故地重游）已交付。
