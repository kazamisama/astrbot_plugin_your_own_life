...
- 状态：blocked（等 ESM `consume_energy` 契约，见 `docs/requirements.md`；上游 API 未提供前不交付）。
...
3. L1 剩余：L1-03 精力预算——blocked（依赖 ESM `consume_energy`，与上游契约同批）；L1-02/04/05/06/07/10 已完成。
